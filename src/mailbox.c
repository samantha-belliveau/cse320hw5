...
void mb_add_message(MAILBOX *mb, int msgid, MAILBOX *from, void *body, int length){
    ...
    MAILBOX_ENTRY *newEntry = malloc(sizeof(MAILBOX_ENTRY));
    newEntry->type = MESSAGE_ENTRY_TYPE;
    (newEntry->content).message = *newMessage;
    newEntry->body = body;
    ...
    if (mb->queue == NULL){
        ...
        mb->queue = malloc(sizeof(QUEUE));
        (mb->queue)->this = newEntry;
        (mb->queue)->prev = NULL;
        (mb->queue)->next = NULL;
...
void mb_add_notice(MAILBOX *mb, NOTICE_TYPE ntype, int msgid, void *body, int length){
    NOTICE *newNotice = malloc(sizeof(NOTICE));
    newNotice->msgid = msgid;
    newNotice->type = ntype;

    MAILBOX_ENTRY *newEntry = malloc(sizeof(MAILBOX_ENTRY));
    newEntry->type = NOTICE_ENTRY_TYPE;
    (newEntry->content).notice = *newNotice;
...
MAILBOX_ENTRY *mb_next_entry(MAILBOX *mb){
    ...
    MAILBOX_ENTRY *toReturn = (mb->queue)->this;

    QUEUE *temp = (mb->queue)->next;
